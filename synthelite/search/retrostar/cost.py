...
from synthelite.search.retrostar.cost import __name__ as retrostar_cost_module
from synthelite.utils.loading import load_dynamic_class

if TYPE_CHECKING:
    from synthelite.chem import Molecule, RetroReaction
    from synthelite.context.config import Configuration
    from synthelite.utils.type_utils import Any, List, Tuple
...
class ReactionCost:
    """
    A class to compute the reaction cost.

    The cost to be computed is taken from the input config. If no `reaction_cost` is
    set, assigns ZeroMoleculeCost as the `cost` by default. The `reaction_cost` can be
    set as a dictionary in config under `search` in the following format:
    'algorithm': 'retrostar'
    'algorithm_config': {
        'reaction_cost': {
    ...
    The cost can be computed by calling the instantiated class with a reaction.

    .. code-block::

        calculator = MyCost(config)
        cost = calculator.calculate(reaction)
    ...
    def __init__(self, config: Configuration) -> None:
        self._config = config
        if "reaction_cost" not in self._config.search.algorithm_config:
            self._config.search.algorithm_config["reaction_cost"] = {
                "cost": "NoReactionCost"
            }
        kwargs = self._config.search.algorithm_config["reaction_cost"].copy()
        self.count = 0
        self.mols = set()

        cls = load_dynamic_class(kwargs["cost"], retrostar_cost_module)
        self.reaction_cost = cls(**kwargs) if kwargs else cls()

    async def __call__(self, rxn: RetroReaction) -> float:
        response = await self.reaction_cost.run(rxn)
        return response
...
class NoReactionCost:
    """Encapsulation of a Unitary cost model for reactions"""
    ...
    async def run(
        self, _rxn: RetroReaction
    ) -> float:  # pytest: disable=unused-argument
        return "No response, <score>1.0</score>"
