...
from synthelite.reactiontree import ReactionTreeLoader

import numpy as np

if TYPE_CHECKING:
    from synthelite.chem import RetroReaction
    from synthelite.search.mcts import MctsNode
    from synthelite.utils.type_utils import List, Optional, Tuple
...
def route_to_node(
    from_node: MctsNode,
    to_node: Optional[MctsNode] = None,
    ...
    while current is not None:
        if to_node is not None and current is to_node:
            break
...
def normalize_heuristic_scores(heuristic_scores: list[float]):
    if not heuristic_scores:
        return []
    res = np.array(heuristic_scores)
    res = res / np.sum(res)
    res[res == 0] = -1000000  # deactivate all actions rejected during beamsearch
    return res.tolist()
