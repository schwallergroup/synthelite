...
from synthelite.utils.image import RouteImageFactory
...
class ReactionTree:
    ...
    def to_dict(self, include_metadata=False) -> StrDict:
        ...
        """
        d = self._build_dict(self.root, include_metadata=include_metadata)
        d["lmdata"] = self.lmdata
        return d
...
class ReactionTreeFromDict(ReactionTreeLoader):
    ...
    def _parse_tree_dict(self, tree_dict: StrDict, ncalls: int = 0) -> UniqueMolecule:
        ...
        metadata = None
        if not tree_dict["in_stock"]:
            metadata = {
                "reactionmetrics": tree_dict.get("reactionmetrics"),
                "conditions": tree_dict.get("conditions"),
            }
        self._add_node(
            product_node,
            depth=2 * ncalls,
