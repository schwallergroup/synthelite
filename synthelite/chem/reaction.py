...
if TYPE_CHECKING:
    from synthelite.chem.mol import UniqueMolecule
...
class RetroReaction(abc.ABC, _ReactionInterfaceMixin):
    ...
    def reversed_mapped_reaction_smiles(self) -> str:
        """
        Get the mapped reaction SMILES (in the forward direction!) if it exists
        :return: the SMILES
        """
        products = self.mol.mapped_smiles
        reactants = ".".join(mol.mapped_smiles for mol in self._products_getter())
        return reactants + ">>" + products
...
class TemplatedRetroReaction(RetroReaction):
    ...
    def to_absolute_reactions(self) -> List[AbsoluteRetroReaction]:
        seen_reactant_strings = set()
        res = []
        for reactants in self.reactants:
            reactant_smi = ".".join(mol.smiles for mol in reactants)
    ...
    def _apply_with_rdchiral(self) -> Tuple[Tuple[TreeMolecule, ...], ...]:
        ...
        """
        # reactants_out = self._recursive_apply_rdchiral(self.mol, self.smarts, d=0)
        # reactants = self._compile_reactants(reactants_out)

        reaction = rdc.rdchiralReaction(self.smarts)
        tgt = _RdChiralProductWrapper(self.mol)
        reactants = rdc.rdchiralRun(reaction, tgt, keep_mapnums=True)
    ...
    def _recursive_apply_rdchiral(
        self, target: TreeMolecule, rxn: str, d=0
    ) -> List[str]:
        """
        This should return a list of strings. Each string is a set of reactants, reparated by '.'
        """
        reaction = rdc.rdchiralReaction(rxn)
        tgt = _RdChiralProductWrapper(target)
        try:
            reactants = rdc.rdchiralRun(reaction, tgt, keep_mapnums=True)
            if reactants and d < 4:
                new_reacts = []
                for r in reactants:
                    new_appl = self._recursive_apply_rdchiral(
                        TreeMolecule(parent=None, smiles=r), rxn, d=d + 1
                    )
                    if new_appl:
                        new_reacts.extend(new_appl)
                    else:
                        new_reacts.append(r)
                return new_reacts
    ...
    def _compile_reactants(self, reactants_out: List[str]) -> List[str]:
        if reactants_out[0] == self.mol.smiles:
            return []
        if len(reactants_out) > 4:
            return reactants_out[:4]
        return reactants_out
...
class TemplatedRetroReactionForMolSet(TemplatedRetroReaction):
    _required_kwargs = ["smarts"]

    def __init__(
        self,
        mol_set: set[TreeMolecule],
        index: int = 0,
        metadata: Optional[StrDict] = None,
        **kwargs: Any,
    ):
        ...
        self.index = index
        self.metadata: StrDict = metadata or {}
        self._products: Optional[Tuple[TreeMolecule, ...]] = None
        self._reactants: Optional[Tuple[Tuple[TreeMolecule, ...], ...]] = None
        self._smiles: Optional[str] = None
        self._kwargs: StrDict = kwargs
    ...
    def _apply(self):
        ...
        for mol in self.mol_set:
            action = TemplatedRetroReaction(
                ...
                metadata=self.metadata,
                smarts=self.smarts,
                use_rdchiral=self._use_rdchiral,
            )
            for reactant_set in action.reactants:
                reactant_sets.append(reactant_set)
                products.append(mol)

        products = tuple(products)
        reactants = tuple(reactant_sets)
    ...
    @reactants.setter
    def reactants(self, value: Tuple[Tuple[TreeMolecule, ...], ...]):
...
class AbsoluteRetroReaction(RetroReaction):
    """Reaction class without any ambiguity in reaction site"""

    # _required_kwargs = ["reactants"]

    def __init__(
        self,
        mol: TreeMolecule,
        # reactants: Optional[Tuple[Tuple[TreeMolecule]]] = None,
        # smarts: Optional[str] = None,
        # mol_smiles: Optional[str] = None,
        # reactants_smiles: Optional[Iterable[str]] = None,
        # retro_action: Optional[TemplatedRetroReaction] = None,
        index: int = 0,
        metadata: Optional[StrDict] = None,
        **kwargs: Any,
    ):
        ...
        # self._mol = mol
        self._reactants = kwargs.get("reactants")
        self._reactants_mapped_smiles = kwargs.get("reactants_mapped_smiles")
        if self._reactants_mapped_smiles is not None:
            self._reactants_mapped_smiles = tuple(self._reactants_mapped_smiles)
        self.smarts = kwargs.get("smarts")

        if self._reactants:
            assert len(self._reactants) == 1, "Only single set of reactants is allowed"

        # self._retro_template = kwargs.get("retro_action")
        # self._mol_smiles = mol_smiles
        # self._retro_template = retro_action

        # if self._mol is None and self._mol_smiles is None:
        #     raise ValueError("Either mol or mol_smiles must be provided")
        # if self._reactants is None and self._reactants_smiles is None:
        #     raise ValueError("Either reactants or reactants_smiles must be provided")

        # self.index = index
        # self.metadata: StrDict = metadata or {}

    # @property
    # def mol(self) -> TreeMolecule:
    #     if self._mol is None:
    #         self._mol = TreeMolecule(parent=None, smiles=self._mol_smiles, sanitize=True)
    #     return self._mol

    @property
    def reactants(self) -> Tuple[TreeMolecule, ...]:
        if self._reactants is None:
            self._reactants = (
                tuple(
                    TreeMolecule(parent=self.mol, smiles=smi, sanitize=True)
                    for smi in self._reactants_mapped_smiles
                ),
            )
        return self._reactants

    @property
    def reactants_mapped_smiles(self) -> Tuple[str, ...]:
        if self._reactants_mapped_smiles is None:
            self._reactants_mapped_smiles = tuple(
                mol.mapped_smiles for mol in self.reactants[0]
            )
        return self._reactants_mapped_smiles

    # @property
    # def retro_action(self) -> Optional[TemplatedRetroReaction]:
    #     return self._retro_template

    @classmethod
    def from_molset_nodes(
        self, parent_node, child_node, action: Optional[TemplatedRetroReaction] = None
    ):
        if action is None:
            action = parent_node[child_node]["action"]

        parent_mols = parent_node.state.mols
        child_mols = child_node.state.mols

        reactant_mols = set(mol for mol in child_mols if mol not in parent_mols)

        mol = action.mol

        for reactants in action.reactants:
            if set(reactants) == reactant_mols:
                return AbsoluteRetroReaction(
                    mol=mol, reactants=(reactants,), retro_action=action
                )
...
class FixedRetroReaction(_ReactionInterfaceMixin):
    ...
    def __init__(
        self,
        mol: UniqueMolecule,
        reactants: Tuple[Tuple[UniqueMolecule, ...], ...] = (),
        smiles: str = "",
        metadata: Optional[StrDict] = None,
    ) -> None:
        self.mol = mol
...
def action_to_str(action, retro=True, canonicalize=False):
    def _canonicalize(smiles: str):
        return Chem.MolToSmiles(Chem.MolFromSmiles(smiles))

    product_str = action.mol.smiles
    reactant_strs = []
    for reactant_set in action.reactants:
        reactant_str = ".".join([r.smiles for r in reactant_set])
        reactant_strs.append(reactant_str)

    if canonicalize:
        product_str = _canonicalize(product_str)
        reactant_strs = [_canonicalize(each) for each in reactant_strs]

    if retro:
        reaction_strs = [
            f"{product_str}>>{reactant_str}" for reactant_str in reactant_strs
        ]
    else:
        reaction_strs = [
            f"{reactant_str}>>{product_str}" for reactant_str in reactant_strs
        ]

    reaction_strs = list(
        dict.fromkeys(reaction_strs)
    )  # deduplicate while preserving orders
    reaction_strs = sorted(reaction_strs)

    return reaction_strs

    # if reaction_mapped:
    #     product_smiles = action.mol.mapped_smiles
    #     reactants_smiles = '.'.join([r.mapped_smiles for r in action.reactants[0]]) if action.reactants else ''
    # else:
    #     product_smiles = action.mol.smiles
    #     reactants_smiles = '.'.join([r.smiles for r in action.reactants[0]]) if action.reactants else ''

    # if canonicalize:
    #     product_smiles = _canonicalize(product_smiles)
    #     reactants_smiles = _canonicalize(reactants_smiles)

    # if retro:
    #     # reverse the reaction for retrosynthesis
    #     reaction_str = f"{product_smiles}>>{reactants_smiles}"
    # else:
    #     reaction_str = f"{reactants_smiles}>>{product_smiles}"

    # return reaction_str


def deduplicate_actions(actions, priors):
    seen_actions = set()
    final_actions = []
    final_priors = []
    ...
    return final_actions, final_priors
