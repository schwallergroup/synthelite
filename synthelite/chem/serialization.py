...
import synthelite.chem
from synthelite.utils.loading import load_dynamic_class

if TYPE_CHECKING:
    from synthelite.chem import RetroReaction
    from synthelite.utils.type_utils import Any, Dict, Optional, Sequence, StrDict
...
class MoleculeSerializer:
    ...
    def __getitem__(self, mol: Optional[synthelite.chem.Molecule]) -> Optional[int]:
        if mol is None:
            return None
    ...
    def _add_mol(self, mol: synthelite.chem.Molecule) -> None:
        id_ = id(mol)
        dict_ = {"smiles": mol.smiles, "class": mol.__class__.__name__}
        if isinstance(mol, synthelite.chem.TreeMolecule):
...
class MoleculeDeserializer:
    ...
    def __getitem__(self, id_: Optional[int]) -> Optional[synthelite.chem.Molecule]:
        if id_ is None:
            return None
    ...
    def get_tree_molecules(
    ...
    ) -> Sequence[synthelite.chem.TreeMolecule]:
        """
        ...
        for id_ in ids:
            obj = self[id_]
            if obj is None or not isinstance(obj, synthelite.chem.TreeMolecule):
    ...
    def _create_molecules(self, store: dict) -> None:
        for id_, spec in store.items():
            ...
            kwargs = dict(spec)
            del kwargs["class"]
            self._objects[id_] = getattr(synthelite.chem, cls)(**kwargs)
...
def deserialize_action(
    ...
    try:
        ...
        class_spec = "synthelite.chem.TemplatedRetroReaction"
