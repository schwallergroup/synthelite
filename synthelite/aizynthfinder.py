...
from synthelite.analysis import (
    RouteCollection,
    RouteSelectionArguments,
    TreeAnalysis,
)
from synthelite.chem import Molecule, TreeMolecule
from synthelite.context.config import Configuration
from synthelite.context.policy import BondFilter
from synthelite.context.scoring import BrokenBondsScorer, CombinedScorer
from synthelite.search.andor_trees import AndOrSearchTreeBase
from synthelite.search.mcts import MctsSearchTree
...
class AiZynthFinder:
    ...
    def __init__(
        ...
        self._steer_query: Optional[str] = None
    ...
    def tree_search(self, show_progress: bool = True) -> float:
        ...
        if show_progress:
            pbar = tqdm(total=self.config.search.iteration_limit, leave=True)
    ...
    def _setup_search_tree(self) -> None:
        ...
        if self.config.search.algorithm.lower() == "mcts":
            ...
            # Also set reference for other tree types if they support it
            if hasattr(self.tree, "_finder"):
                self.tree._finder = self
